...
<<<<<<< HEAD:Src/quality-check-03a.Rmd
```{r loop}
for (i in cd_list) {
  county_plot(co, cd, i)
  bg_list <- extract_bg(bl_counts, i)
  for (j in bg_list) {
    display_blocks(bl_counts, i, j)
    bg_plot(cd, i, bg, bg_counts, j)
  }
=======
plot_co <- function(co, cd, i_cd) {
  ...
  ggplot(co, aes())                     +
    ...
    geom_sf(
      ...
      color=NA)               -> co_map
  plot(co_map)
>>>>>>> a2881dbfb96d1f7b1f05fc3c0d9c61e5b017274f:Src/quality-check-03.Rmd
}
```
